...
- **Outer `CatHeadPlacements` MovieClip display-list accumulation as the missing-anchor source**: Direction 48 decoded `char_id=11007` (1505 frames) in `catparts.swf`. Frame 0 places all 8 anchors at fixed depths; subsequent frames update only depth=1/2/27. Spec-compliant accumulation produces **identical anchor sets for frame 99 (Kami clean) and frame 304 (Whommie defect+)**, contradicting the Direction 47 outer-clip hypothesis. The placement-driven model still stands but must be refined — the per-frame depth=1 sub-clip is the remaining candidate.
