...
| `FUN_140734760`  | Visual bone/transform placement and `CatPart+0xC` "anchor populated" sub-flag setter. Fetches `"CatHeadPlacements"` MovieClip via `FUN_1409a5bb0(0x73, "CatHeadPlacements")`, calls `FUN_140996b80(clip, *(int*)(CatData+0x84) - 1)` to seek frame=headShape-1, then flat-iterates `clip+0xb0` for `clip[+0xac]` items reading each child's name from `child+0x48`. Clears at entry, then sets `CatPart+0xC` to 1 (NOT `+0x18` — Direction 50 correction) for matching anchor names (`"leye"`, `"reye"`, eyebrows copied from eyes at loop end, `"lear"`, `"rear"`, `"mouth"`, `"ahead"`, `"aneck"`, `"aface"`). Writes at CatData byte offsets 0x290, 0x2e4, 0x338, 0x38c, 0x3e0, 0x434, 0x488, 0x4dc, 0x530, 0x584. Also copies bone transform data (position, rotation, scale) from each matching child into the corresponding CatPart transform fields. Direction 50. |
