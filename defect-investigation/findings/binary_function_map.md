...
| `FUN_140734760`  | Visual bone/transform placement AND selective flag setter. Loads `"CatHeadPlacements"` GON using current head ID, clears facial/attached present flags, then sets `CatPart+0x18` back to 1 only for anchors present in the placement entry (`"leye"`, `"reye"`, eyebrows copied from eyes, `"lear"`, `"rear"`, `"mouth"`, `"ahead"`, `"aneck"`, `"aface"`). |
