...
1. **Direction 54 — Find every writer of CatPart+0x18 (any value, not just zero).** Ghidra task. The byte is read by the renderer but never serialized, so something writes it at runtime — including whatever sets it to its default nonzero. Search across: (a) CatData constructor / zero-fill / spawn paths, (b) every callee of `FUN_14022dfb0` between deserialize return and the `FUN_140734760` invocation, (c) any post-deserialize "cook" / "post-load" function. Look for stores to displacements `0xa4, 0xf8, 0x14c, 0x1a0, ...` (CatData+0x8c+k*0x54+0x18 for k=0..13) on a CatData*.
2. **Direction 55 (parallel candidate) — Diff Whommie's vs. Kami's serialized CatPart blocks.** Whommie and Kami share eye 139 and eyebrow 23 base IDs but only Whommie shows the defect. Compare the 14 × 0x54-byte CatPart records byte-by-byte between Whommie and Kami; any sub-offset that differs in the eye/eyebrow slots is a candidate input to whatever sets `+0x18`. Same for Bud vs. a control with the same ear ID 132 if one exists.
3. **Re-examine FUN_140734760 for +0x18 writes specifically.** Direction 50 showed it writes +0x0C; Direction 51 said no zero-writers to +0x18 found, with Low confidence. The agent in Direction 53 raised the question whether the head-placement walker may also touch +0x18 via an indirect access pattern Ghidra rendered as a different offset. Worth a fresh decompile pass focused only on +0x18-shaped accesses.
4. **Prototype parser-side reconstruction.** Once a stable on-disk defect signal is identified for the Whommie/Bud class, synthesize visual defect entries from the appropriate field.
