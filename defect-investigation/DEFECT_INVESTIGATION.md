...
Direction 44 tightened the `FUN_1400ca4a0` lead:
- The investigation save has `save_file_percent = 80` and `save_file_next_cat_mutation = 90`, so the save-percent `FUN_1401d2ff0` hook returns early and performs zero `FUN_1400ca4a0` calls.
- `_DAT_141130700`, the `FUN_1400ca4a0(..., param_2=1)` birth-defect candidate table, is `(0, 1, 2, 3, 3, 3, 3, 3, 8, 8)`.
- Via `FUN_1400cb130`, those CatPart IDs map only to body, head, tail, rear legs, and front legs/arms.
- Therefore the save-percent `ca4a0` path cannot directly produce Whommie's eye/eyebrow defects or Bud's ear defect. Keep focus on the runtime missing-flag reconstruction, breeding inheritance, or a different display/effect path.
...
**Direction 46 resolution (2026-04-26):** The offset puzzle is resolved. `FUN_1400a5390` and `FUN_1400c9810` read the same missing-part flag byte at `CatPart+0x18`; the apparent `+0x24` came from comparing container-relative serializer offsets to CatData-relative display offsets without adding the CatParts container base (`CatData+0x60`). `FUN_1400a6790` calls `FUN_1400a5390` with CatData-relative slot bases `+0x8c, +0xe0, +0x134, +0x188, +0x1dc, +0x230, +0x284, +0x2d8, +0x32c, +0x380, +0x3d4, +0x428, +0x47c, +0x4d0`; these are the same records `FUN_14022ce10` serializes at container-relative bases `+0x2c, +0x80, +0xd4, ...`. At those bases, `FUN_1400c9810` reads category at `base+0x00`, part id at `base+0x04`, and the missing flag at `base+0x18`. `FUN_14005dfd0` initializes those same flag bytes to `1` (present), including CatData offsets `+0xa4, +0xf8, +0x14c, +0x1a0, +0x1f4, +0x248, +0x29c, +0x2f0, +0x344, +0x398, +0x3ec, +0x440, +0x494, +0x4e8`. `FUN_14022cd00` only serializes record `+0x04..+0x14`, so the runtime present/missing flag at record `+0x18` is still not in the saved T array. See `tools/field_mapper/direction46_results.txt`.

Direction 46 also checked the obvious mutation route and load-time default-init call. `FUN_1400ca4a0 -> FUN_1400caa20 -> FUN_1400cb130` writes part IDs but does not write the `CatPart+0x18` flag. `FUN_14022dfb0`'s call to `FUN_1400b5260` sets `RCX=CatData*`, `R8D=3`, and `R9D=0`; `RDX` is not deliberately set at that call site. The decompiled four-argument signature is plausible, but load-time default init appears to run with `param_4=0`, unlike breed-time init from `FUN_1400a6790` with `param_4=1`.

**Corrected open question after Direction 46:** What sets the runtime slot `CatPart+0x18` flag to zero for stable missing-part defects after load? The next binary step is to trace setters/writers of the runtime flag bytes (`CatData+0xa4, +0xf8, +0x14c, ...`), not to re-scan the saved blob.

**Direction 47 — post-load placement reconstruction is the first confirmed missing-flag setter (2026-04-26):**

Advisor-strategy review traced `FUN_1400b5260` mode behavior and then rechecked its child/post-load calls. `FUN_1400b5260` itself does **not** directly write `CatPart+0x18` missing flags to zero. Its apparent signature is `FUN_1400b5260(CatData*, unused/noisy arg2, int gender_or_mode, char use_save_context_mode)`: `gender_or_mode` controls sex/gender initialization, and `use_save_context_mode` changes a class/default-data lookup path, not the body-part present flags.

The important correction is `FUN_140734760`. It is not placement-only. It loads `"CatHeadPlacements"` using the current head/placement ID, clears facial/attached present flags, and sets specific `CatPart+0x18` flags back to `1` only when the selected placement entry contains matching anchors:

- `"leye"` / `"reye"` set left/right eye present flags.
- eyebrow records are copied from the eye records at loop end, explaining why missing eyes can also produce missing eyebrows.
- `"lear"` / `"rear"` set left/right ear present flags.
- `"mouth"`, `"ahead"`, `"aneck"`, and `"aface"` set other placement-driven part flags.

This gives the first confirmed post-load mechanism that can produce stable runtime missing-part defects without any serialized per-cat flag. The fixed investigation save supports the model for Whommie/Kami: Whommie has headShape `304` while clean control Kami has headShape `99`, even though both share eye `139` and eyebrow `23`. Bud has headShape `319` and ear `132`; the exact control comparison for head `319` still needs the placement table decoded.

The placement table is not plain GON text. The strings `"CatHeadPlacements"`, `"leye"`, `"reye"`, `"lear"`, `"rear"`, `"mouth"`, `"ahead"`, `"aneck"`, and `"aface"` were found in `game-files/resources/gpak-video/swfs/catparts.swf`. See `tools/field_mapper/direction47_b5260_mode_flags_report.txt` and `tools/field_mapper/direction47_review_results.txt`.

**Updated working model after Direction 47:** The unresolved defects are likely derived from saved visual/head IDs through post-load CatHeadPlacements reconstruction. The parser can probably reproduce them by decoding enough of `catparts.swf` to know which head placement entries omit eye/eyebrow/ear anchors, then adding synthetic `0xFFFFFFFE` defect entries for the affected slots.

**Next concrete steps (priority order):**

1. **Decode CatHeadPlacements from `catparts.swf`.** Extract anchor names for headShape `304`, `99`, and `319`, then compare Whommie/Kami/Bud and any controls with matching head IDs.
2. **Prototype parser-side reconstruction.** Once anchor omissions are confirmed, synthesize the corresponding `0xFFFFFFFE` visual defect entries from saved headShape/placement data instead of adding any new save-field scan.
...
4. ~~**Resolve the +0x18 vs +0x24 puzzle empirically.**~~ **CLOSED (Direction 46).** Runtime display and breeding both read `CatPart+0x18`; the earlier `+0x24` interpretation used the wrong record bases.
5. ~~**Row-audit the .sav file's other SQLite tables for per-cat keys**~~ — **CLOSED (Direction 44).** All 5 tables and all 11 files blobs fully row-audited. No per-cat defect data exists anywhere in the save file. The entire on-disk save is exhausted.
...
- `FUN_1400a6790`: `glaiel::CatData::breed(...)`. This is the executable-side birth-defect generation lead. It calls `FUN_1400c17f0` with `"birth_defects"` and `FUN_1400ca4a0` with `"birth_defect"`. It calls `FUN_1400a5390` with CatData-relative slot bases `+0x8c, +0xe0, ...`, which match the serialized record bases after adding the CatParts container base (`CatData+0x60`).
- `FUN_1400a5390`: body-part inheritance helper. For each body part, it looks up `(part_category, part_id)` from the two parents. If the byte at `CatPart+0x18` is zero, it substitutes part ID `0xFFFFFFFE` for the lookup. It then writes the selected part ID to the child at `CatPart+0x04`. This explains how runtime code can use GON block `-2` without saving a literal `0xFFFFFFFE`.
- `FUN_1400c9810`: effective mutation/display-list builder. Direction 46 confirms it reads the same runtime slot flag as `FUN_1400a5390`: category at CatPart `base+0x00`, part id at `base+0x04`, and missing flag at `base+0x18`.
- `FUN_14005dfd0`: body-part container constructor. Direction 46 confirms it initializes the runtime display/breeding slot `CatPart+0x18` bytes to `1` (present).
...
- `FUN_1401d2ff0` as the per-cat save loader (Direction 43/44 correction): this function is `GlobalProgressionData::ComputeSaveFilePercentage`, applying progression-milestone mutations from save % vs `next_cat_mutation` SQLite key — not the per-cat defect applier. In the investigation save, `save_file_percent = 80` and `save_file_next_cat_mutation = 90`, so this path is gated off entirely. Even when active, its `FUN_1400ca4a0(..., param_2=1)` candidate table only covers body/head/tail/rear legs/front legs, not eyes/eyebrows/ears.
- Direction 33's per-record field labels (Direction 45/46 refinement): the serialized offsets are correct (`FUN_14022cd00` writes 5 u32s at serialized record `+0x04..+0x14`). Direction 46 shows the runtime display/breeding offsets are the same records viewed CatData-relative rather than container-relative. `FUN_1400a5390` and `FUN_1400c9810` both read missing flag `CatPart+0x18`, which is immediately after the serialized five-u32 window and is not serialized in T.
