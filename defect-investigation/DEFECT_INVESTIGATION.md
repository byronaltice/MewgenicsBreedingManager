...
**Direction 57 update (2026-04-29) — INVESTIGATION RESOLVED.** SWF anchor re-decode of `CatHeadPlacements` with proper RemoveObject2 (tag 28) handling, seeking to `headShape-1` (the actual frame index `FUN_140734760` requests via `FUN_140996b80(clip, headShape-1)`):

- **Frame 303 (Whommie's seek):** `reye` is absent at depth=70 — removed by a RemoveObject2 tag at frame 303; re-placed at frame 304 only. Predicts `CatPart[8]+0x18=0` → emits `0xFFFFFFFE` for right eye → Eye Birth Defect (Blind), with Eyebrow Defect via Direction 54's eye→eyebrow propagation. **Matches observed defects.** ✓
- **Frame 318 (Bud's seek):** `lear` (depth=31) and `rear` (depth=35) both absent — both RemoveObject2'd at frame 318, re-placed at 319. Predicts both ear CatParts get +0x18=0 → Ear Birth Defect (-2 DEX). **Matches observed defects.** ✓
- **Frame 98 (Kami's seek):** all 8 anchors present. **No defects predicted, none observed.** ✓
- **Why Direction 48 missed it:** Direction 48 checked frames 99/304/319 (headShape direct), at which the re-placement tags ARE committed and the "identical anchor sets" conclusion was correct *for those frames*. But the runtime walks frames `headShape-1`. Off-by-one in the comparison frame, not in the SWF or the runtime. Direction 48 also did not handle RemoveObject2 — though for the head shapes Whommie/Bud sit on, the tag-stream encoding pattern is "remove at frame=N-1, re-place at frame=N", so seeking to N-1 vs N produces different anchor sets, while seeking to either frame far from a removal boundary is unaffected.
- Script: `scripts/investigate-direction/investigate_direction57.py`. Audit: `audit/direction/direction57_swf_anchor_redecode_results.txt`.

**Parser-side fix is now well-defined:** for each cat, parse `CatHeadPlacements` cumulatively to frame `headShape-1` with full RemoveObject2 handling, check anchor presence for `leye/reye/lear/rear/mouth/ahead/aneck/aface`, synthesize the corresponding defect entries (with eye→eyebrow propagation per Direction 54).

**Direction 54/55/56 update (2026-04-29) — RUNTIME PIPELINE FULLY MAPPED.** Three parallel dispatches resolved the missing-defect runtime mechanism end-to-end.

- **Direction 54 [PIVOTAL CORRECTION]: `FUN_140734760` IS the `CatPart+0x18` writer.** Direction 50's "writes +0xC" reading was wrong — it didn't account for `param_1` being `double*` (8-byte stride). Address arithmetic: `*(undefined1 *)(param_1 + 0x52) = 0` with `param_1 = CatData+0x60` gives byte offset `0x52*8 = 0x290` from param_1 = `CatData+0x2f0` = `CatPart[7]+0x18` (matches CatPart[7] base CatData+0x2d8 + 0x18). The function unconditionally zeros `CatPart[k]+0x18` for the head anchor slots at entry, then sets each to 1 only if the corresponding anchor name (`leye`, `reye`, `lear`, `rear`, `mouth`, `ahead`, `aneck`, `aface`) is present in the headShape frame's `CatHeadPlacements` display list. **Eyebrow propagation:** at loop end, `CatPart[9]+0x18 ← CatPart[7]+0x18` and `CatPart[10]+0x18 ← CatPart[8]+0x18`, so eye and eyebrow defects always co-occur — explains Whommie's combined Eye+Eyebrow defect pattern. The renderer `FUN_1400c9810` reads the same +0x18 byte and emits `0xFFFFFFFE` when zero. Constructor `FUN_14005dfd0` defaults all CatPart+0x18 to 1. So the complete pipeline is: constructor sets all to 1 → `FUN_140734760` rewrites k=7..13 based on SWF anchor presence → renderer gates display on +0x18. See `audit/direction/direction54_part18_writer_hunt_results.txt`. `findings/binary_function_map.md` corrected.

- **Direction 55: CatPart 5-tuple holds no defect-differentiating signal on disk.** Whommie vs Kami diff across all 14 slots: `+0x04`, `+0x0C`, `+0x10`, `+0x14` are identical or zero in eye/eyebrow slots. Only `+0x08` differs, and it's the per-cat texture/fur echo (uniform across all 14 slots — same value at every k). Same null result for Bud vs Murisha in ear slots. Whommie's `+0x08 = 706` lands in the hardcoded defect range and is already detected as Fur Birth Defect, but explains nothing about eye/eyebrow defects. **Implication:** the on-disk input that distinguishes Whommie from Kami at the eye slot must live OUTSIDE the CatPart records — and combined with Direction 54, the only on-disk input `FUN_140734760` reads is `*(int*)(CatData+0x84)` (saved `headShape`). See `audit/direction/direction55_catpart_diff_results.txt`.

- **Direction 56 [verification]: SWF runtime is spec-compliant (CONFIRM).** Independent re-decompile of `FUN_140996b80` and `FUN_140997590`. Frame-walker is strictly cumulative; accumulator merges types 1/2/3/4 per spec, skips 5/6; lazy single-init at clip+0xd0[+0xc8]; no cat-specific state branches. **Important nuance:** the "frames 99 and 304 produce identical anchor sets" claim is a *data* claim about the SWF, not a code claim. The runtime would faithfully apply RemoveObject (type 3) tags between frames 100–304 if any target anchor depths. See `audit/direction/direction56_swf_runtime_verification_results.txt`.

**Single remaining question (much narrower):** Why does `headShape=304`'s frame-cumulative anchor set lack `leye`/`reye` while `headShape=99`'s does not? Direction 48's spec decode said the anchor sets were identical; Direction 56 says the runtime is spec-compliant; Direction 54 says everything else about the pipeline is now solved. Either Direction 48 missed RemoveObject tags between frames 100–304 in the SWF tag stream targeting anchor depths, or Direction 48's decode mishandled some tag type. The fix (parser-side reconstruction) is now well-defined: replicate the SWF cumulative anchor walk for any cat's headShape, mark eye/eyebrow/ear/mouth defects when the corresponding anchor is absent.

**Direction 53 update (2026-04-28):** Disk scan negative — there are two defect storage mechanisms, and Whommie/Bud belong to the harder one. Across all 947 cats, **17 hold `0xFFFFFFFE` at some CatPart[k]+0x04** on disk, and **the existing parser already detects every one of them correctly** via the T-array sentinel path (e.g. Flekpus, Scronc, Katy Kat). **Whommie and Bud hold ordinary base-part IDs at every CatPart+0x04** — no sentinel. So `FUN_1400a5390`'s breed-time `0xFFFFFFFE`-write mechanism is real but not what produces Whommie/Bud's defects. Their defect must originate from a non-+0x04 on-disk signal that something post-load translates into `CatPart+0x18 = 0`. See `audit/direction/direction53_part04_disk_scan_results.txt`. **Suggestive pattern:** Whommie's missing defects (eye, eyebrow) and Bud's (ear) are exactly the head-placement anchor slots `FUN_140734760` walks — but Direction 50 established that frames 99 (Kami) and 304 (Whommie) produce *identical* anchor sets, so frame-seek alone cannot differentiate them. The mechanism is still hidden.
...
1. **All defect-mechanism questions are resolved.** Pipeline: SWF tag stream of `CatHeadPlacements` per `headShape-1` → `FUN_140734760` writes `CatPart+0x18` → `FUN_1400c9810` emits `0xFFFFFFFE` defect placeholder when zero. Direction 57 located the specific RemoveObject2 events that produce Whommie's and Bud's missing anchors at their seek frames.

2. **Implementation question (now the only thing in front of a parser fix):** the predicted defects match observed at frames 303 (Whommie) and 318 (Bud) with the assumption that the runtime's gotoFrame uses inclusive `fn ≤ target` semantics. Direction 56 verified the accumulator is spec-compliant; the inclusive-seek assumption could be confirmed cheaply by re-checking `FUN_140996b80`'s loop bound. Not strictly needed — the empirical match is strong corroboration.

3. **RNG paradox (Direction 45):** Every cat enters `FUN_1400b5260` with the same TLS state. How do cats get different body parts? One of: (i) undiscovered 4th arg carrying cat-specific state, (ii) a non-restoring branch in the wrapper, (iii) the cache check bypasses the RNG-restoring path for some callers. (Orthogonal to the missing-defect parser fix; can stay open.)

---

## Best Path Forward (priority order)

1. **Build the parser-side defect synthesizer.** Promote `investigate_direction57.py`'s SWF cumulative walker into a reusable module under `src/` (or `src/breed_priority/` if the operator prefers). For each loaded cat, seek `CatHeadPlacements` to `headShape-1`, check anchor presence for `{leye, reye, lear, rear, mouth, ahead, aneck, aface}`, synthesize defect entries when absent. Apply Direction 54's eye→eyebrow propagation: lbrow follows leye, rbrow follows reye. Wire into `save_parser.py` so defects appear in `Cat.visual_mutation_entries` like the existing 17 sentinel-path cats.
2. **Roster validation.** Run the synthesizer across all 947 cats; cross-check against any in-game defect list the operator has, and against the parser's existing detections to ensure we don't duplicate-flag the 17 sentinel-path cats.
3. **Optional Ghidra confirmation** of `FUN_140996b80`'s inclusive seek semantics — cheap, would raise confidence from "empirical match" to "mechanism fully verified". Not blocking.
4. **Resolve the RNG paradox** (lower priority — orthogonal).
