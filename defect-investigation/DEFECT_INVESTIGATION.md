...
1. **Why does headShape=304's SWF frame lack `leye`/`reye` while headShape=99's does not?** This is now the only open question for the missing-defect mechanism. Direction 54 mapped the full runtime pipeline (constructor defaults +0x18=1, `FUN_140734760` zeros then sets based on anchor presence in the headShape frame, renderer gates display). Direction 56 verified the SWF runtime is spec-compliant. Direction 48's spec decode said frames 99 and 304 produce identical anchor sets — but Direction 56 flagged that as a *data* claim, and the runtime would faithfully apply RemoveObject tags if present in the tag stream between frames 100–304. Either Direction 48 missed RemoveObject (or other tag-type) handling, or there's something else cat-specific feeding the anchor walk that we haven't identified.

2. **Closed (Directions 54/55):** What writes CatPart+0x18, where in the corridor does +0x04 land — answered. The +0x18 writer is `FUN_140734760` itself (not a separate function). The +0x04 corridor was traced and Direction 53 confirmed Whommie/Bud have normal part IDs there.

3. **RNG paradox (Direction 45):** Every cat enters `FUN_1400b5260` with the same TLS state. How do cats get different body parts? One of: (i) undiscovered 4th arg carrying cat-specific state, (ii) a non-restoring branch in the wrapper, (iii) the cache check bypasses the RNG-restoring path for some callers. (Lower priority — orthogonal to the missing-defect parser fix.)

---

## Best Path Forward (priority order)

1. **Direction 57 — Re-decode Whommie's headShape=304 frame in the SWF.** Walk the `CatHeadPlacements` (`char_id=11007`) tag stream cumulatively from frame 0 to frame 303, applying ALL display-list operations (PlaceObject types 1/2/4 AND **RemoveObject type 3**), and report the depth-keyed display list at frame 303. Compare to the same walk for frame 98 (Kami's headShape=99, 0-indexed). Specifically check whether any anchor-depth child placed at frame 0 is removed before frame 304. Direction 48's prior decode may have only handled PlaceObject and missed RemoveObject — that would explain the "identical anchor sets" finding being incorrect. This is a text-resources / SWF-decode task, not a Ghidra task.
2. **If Direction 57 confirms missing anchors:** prototype parser-side reconstruction. For each cat, decode its `CatHeadPlacements`-frame-(headShape-1) anchor presence in Python, and synthesize defect entries when expected anchors (`leye`, `reye`, `lear`, `rear`, `mouth`) are absent. Eye-absent ⇒ also synthesize eyebrow defect (per Direction 54's eyebrow propagation).
3. **If Direction 57 contradicts** (anchors really are present at frame 304): re-examine `FUN_140734760` for any non-anchor-name input to its `+0x18` decisions — maybe a secondary read of cat data or a per-frame transform that influences whether a child counts as "present".
