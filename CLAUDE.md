...
- *Direction 7 — Parallel variant array search:* Sub-hypotheses 7a, 7b, 7c, 7d, 7e, 7f all ruled out (see below).
  ...
  **Next concrete steps to try:**
  - *Direction 4a — TypeScript editor WRITE path.* The TypeScript save editor (`michael-trinity/mewgenics-savegame-editor`) successfully edits defects — its write/set functions must encode "No Part" defects somehow. Fetch and read the write/mutation-set code from that repo to see if it writes a value beyond T[index+0] to encode the "No Part" state. This is the most targeted remaining lead.
