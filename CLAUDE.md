...
- *Direction 7 — Parallel variant array search:* Sub-hypotheses 7a, 7b, 7c, 7d, 7e, 7f all ruled out (see below).
  ...
  **Next concrete steps to try:**
  ...
  - *Direction 16 — `properties` table exploration.* 259 key+integer rows of world-state, not yet examined for per-cat defect flags. Dump the keys to see if any are cat-specific.
