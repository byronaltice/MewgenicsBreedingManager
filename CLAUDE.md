...
- *Direction 4 — GPAK text string reverse-lookup:* Search `data/text/combined.csv` (6.7MB) and mutation GON files in the GPAK for exact in-game displayed strings ("Blind.", "-2 CHA", "-2 DEX"). Trace back to IDs; determine what save-side value selects them at runtime.
- *Direction 5 — Pre-T blob head (uncharacterized):* The blob head before the pre-T block is ~70–78 bytes (pre_t offset is 0x0046 for Bud, 0x004e for Whommie). This region has never been characterized. It may contain per-cat fields that encode slot-level flags.
- *Direction 6 — Structured parse of abilities/move arrays between T-end and DefaultMove:* Raw byte diffing found 38 hits for Bud in this window but 0 shared with Whommie. This region likely holds ability and move arrays. A structured parse of those arrays (as arrays of IDs rather than raw bytes) has not been attempted and may surface defect-correlated values.
