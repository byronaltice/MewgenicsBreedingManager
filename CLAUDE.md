...
- *Direction 7 — Parallel variant array search:* Sub-hypotheses 7a, 7b, 7c, 7d, 7e, 7f all ruled out (see below).
  ...
  **Next concrete steps to try:**
  ...
  - *Direction 16 — `properties` table exploration.* The save has 259 rows in the `properties` table (key+integer, world-state). These haven't been examined for per-cat defect flags. Unlikely but not yet ruled out. Dump the keys to see if any are cat-specific.
  - *Direction 4 — GPAK text string reverse-lookup:* Still pending.
