...
- *Direction 4 — GPAK text string reverse-lookup:* Search `data/text/combined.csv` (6.7MB) and mutation GON files in the GPAK for exact in-game displayed strings ("Blind.", "-2 CHA", "-2 DEX"). Trace back to IDs; determine what save-side value selects them at runtime. Low priority — tells us what text the game *displays*, not where the defect is *stored*.
- *Direction 7 (new) — T array length beyond 72:* The parser reads exactly 72 u32s for T. The final body-part slot (mouth, index 68) has its T[+4] element at T[72] which is never read. There may be additional T elements beyond 72 encoding defect state. Confirmed observation: GON IDs 139 (eyes), 23 (eyebrows), 132 (ears) do not exist in any GPAK mutation file — they are pure cosmetic base shapes with no mutation entry. This means the game stores the "No Part" defect flag somewhere independent of T[index+0]. T[72+] is untried.
