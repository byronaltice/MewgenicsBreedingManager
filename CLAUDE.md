...
- *Direction 7 — Parallel variant array search:* Sub-hypotheses 7a, 7b, 7c, 7d, 7e, 7f all ruled out (see below).
  ...
  **Next concrete steps to try:**
  - *Direction 4 — External reverse-lookup / code search.* Strongest open lead. Search for primary-source code or modding notes that describe how body-part variants / "No Part" inheritance is serialized.
  ...
  - *Direction 15 — Reconsider the runtime-only hypothesis.* After exhausting all known binary locations, the "No Part" defect may be reconstructed at load time from pre-T seed values (f64[0..7]) via a deterministic defect-roll algorithm, or from parent genetics. Previously dismissed but all alternatives are now exhausted. Seed values for Whommie: f64[0]=0.5976, f64[1]=0.0221, f64[2]=NaN, f64[3]=0.0, f64[4]=0.6553, f64[5]=NaN, f64[6]=0.0, f64[7]=1.014.
  - *Direction 16 — `properties` table exploration:* **RULED OUT.** Script `tools/field_mapper/investigate_direction16.py`, results in `tools/field_mapper/direction16_results.txt`. Current save has **262** `properties` rows (earlier `259` count was stale). Keys cluster into global/world-state groups like `NPCRSTRACKER_*`, `map_*`, `sfunlockcounter_*`, `mapflag_*`, `unlock_*`, `SeenLevelupActiveAbilitiesCounter_*`, `house_*`, and `catinfo_unlock_*`. No keys mention target cat names, target UIDs, or target db_keys. The only superficially cat-like key is `Legacy_Marshmallow_StolenCatID = -1`, which appears to be a single global event slot rather than a per-cat registry. Verdict: `properties` is global/world-state only and not a plausible storage location for Whommie/Bud's missing-part birth defect flags.
