...
4. **Row-audit the SQLite `properties` and `pedigree` tables for per-cat keys** as a fallback if executable tracing doesn't pin down the source. Directions 7c/13b only checked schema; per-cat rows for Whommie/Bud may exist that clean controls don't.
